...
#include <msg_utils/obsData.h>
#include <msg_utils/obsArray.h>

using namespace std;
using namespace Eigen;

ros::Publisher _all_map_pub;
ros::Publisher _obs_data_pub;
...
void RandomMapGenerate()
{
    ...
    msg_utils::obsData obs;
    double a = 0.2, b = 0.2, c = 2.0;
    ...
    else
    {
        for (int i = 0; i < _obs_num; i++)
        {
            ...
            if (is_kdtree_empty == false)
            {
                ...
                {
                    if (sqrt(pointSquaredDistance[0]) < 1.2)
                        continue;
                }
            }
            ...
            for (float theta = 0; theta < 2 * M_PI; theta += 0.1)
            {
                ...
                {
                    pt_random.x = x + a * cos(theta) * sin(phi);
                    pt_random.y = y + b * sin(theta) * sin(phi);
                    pt_random.z = z + c * cos(phi);
                    cloudMap.points.push_back(pt_random);
                }
            }
            msg_utils::obsData obs;
            ...
            obs.y = y;
            obs.z = z;
            obsArray.obsArray.push_back(obs);
        }
    }
...
void pubSensedPoints()
{
    ...
    _obs_data_pub.publish(obsArray);
}
...
int main(int argc, char **argv)
{
    ...
    _obs_data_pub = n.advertise<msg_utils::obsArray>("obs_data", 1);
